    ...
    public class CartService : ICartService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            var cart = await _cartRepository.GetByUserIdAsync(
                userId,
                cancellationToken);
            return cart?.ToResponse() ?? new CartResponse();
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var cart = await GetOrCreateCartForUpdateAsync(userId, cancellationToken);
        ...
        private async Task<Cart> GetOrCreateCartForUpdateAsync(
            Guid userId,
