    ...
    public class UploadService : IUploadService
    {
            ...
            Guid? actorUserId = null)
        {
            ...
            try
            {
                ...
                {
                    await _imageStorage.SaveAsync(fileName, stream, cancellationToken);
                }
            ...
            {
                await DeleteGeneratedFileAsync(fileName);
            ...
            Guid? actorUserId = null)
        {
            ...
            await DeleteStoredFileAsync(imageUrl);
        }
        ...
        private async Task<string> ValidateImageAsync(IUploadFile file, CancellationToken cancellationToken)
        {
        ...
        private async Task DeleteStoredFileAsync(string imageUrl)
        {
            try
            {
                await _imageStorage.DeleteByUrlAsync(
                    imageUrl,
        ...
        private async Task DeleteGeneratedFileAsync(string fileName)
        {
            try
            {
                await _imageStorage.DeleteByNameAsync(
                    fileName,
                    CancellationToken.None);
            }
            catch (IOException ex)
            {
                _logger.LogWarning(ex, "Could not delete image file {FileName}.", fileName);
            }
            catch (UnauthorizedAccessException ex)
            {
                _logger.LogWarning(ex, "Could not delete image file {FileName}.", fileName);
